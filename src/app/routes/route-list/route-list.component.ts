...
export class RouteListComponent implements OnInit, OnDestroy{
  ...
  displayedColumns: string[] = ['path','id'];
  ...
  constructor(public routeServices : RouteServices, public dialog: MatDialog){}

  //on init gets all data to be shown on table
  ngOnInit(){
    this.isLoading = true;
    //calls the getall service to retrieve all data sending pagination parameters
    this.routeServices.getAll(this.pageSize,this.page);
    //calls the update service and subscribes
    this.sub = this.routeServices.getUpdate().subscribe((results: { data: Route[], count: number })=>{
  ...
  onDelete(id: string, path: string){
    const dialogRef = this.dialog.open(DialogComponent, {
      width: '350px',
      data: "Are you sure to delete "+ path +"?"
    });
