...
export class PhaseListComponent implements OnInit, OnDestroy{
  ...
  dataSource = new MatTableDataSource(this.phases);

  //constructor using services and dialog for delete event
  constructor(public phaseServices : PhaseServices, public dialog: MatDialog){}

  //on init gets all data to be shown on table
  ngOnInit(){
    this.isLoading = true;
    //calls the getall service to retrieve all data sending pagination parameters
    this.phaseServices.getAll(this.pageSize,this.page);
    //calls the update service and subscribes
    this.sub = this.phaseServices.getUpdate().subscribe((results: { data: Phase[], count: number })=>{
      this.isLoading = false;
      this.phases = results.data;
      ...
      this.dataSource.filterPredicate = (data, filter) => {
        ...
        const dataStr = data.description;
  ...
  onDelete(id: string, name: string){
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if(result) {
        this.phaseServices.delete(id).subscribe(()=>{
