...
export class PermissionListComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = ['role', 'route','status','id'];
  ...
  constructor(public permissionServices : PermissionServices, public dialog: MatDialog){}

  //on init gets all data to be shown on table
  ngOnInit(){
    this.isLoading = true;
    //calls the getall service to retrieve all data sending pagination parameters
    this.permissionServices.getAll(this.pageSize,this.page);
    //calls the update service and subscribes
    this.sub = this.permissionServices.getUpdate().subscribe((results: { data: Permission[], count: number })=>{
      ...
      this.dataSource.filterPredicate = (data, filter) => {
        ...
        const statusText = data.status ? "Allowed" : "Denied"
        const dataStr = data.role.name + data.route.path + statusText;
  ...
  onDelete(id: string, name: string){
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if(result) {
        this.permissionServices.delete(id).subscribe(()=>{
