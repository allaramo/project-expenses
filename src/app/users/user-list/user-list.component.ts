...
export class UserListComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = ['email','role','status','logged','id'];
  ...
  ngOnInit(){
    ...
    this.userServices.getAll(this.pageSize,this.page);
    //calls the update service and subscribes
    this.sub = this.userServices.getUpdate().subscribe((results: { data: User[], count: number })=>{
      ...
      this.dataSource.filterPredicate = (data, filter) => {
        ...
        const dataStr = data.email + data.role?.name + data.status + data.logged;
  ...
  onDelete(id: string, email: string){
    const dialogRef = this.dialog.open(DialogComponent, {
      width: '350px',
      data: "Are you sure to delete "+ email +"?"
    });
